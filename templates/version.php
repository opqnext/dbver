...
<div class="container">
    ...
    <div>
        <p class="bg-warning text-danger" style="height:35px;line-height:30px;padding-left:10px;font-size:16px;">
            <span>您本地的版本落后于公共版本<?=$this->fall?>次修改</span>
            <a href="<?=U("Version", "update")?>" style="float:right;margin-right:20px;">立即更新</a>
        </p>
    </div>
    <?php endif; ?>
    <div>
        <?php if($this->diffFormat['new']):?>
        <div class="v_content">
            ...
            <?php endforeach; ?>
                <form class="form-inline" action="<?=U("Version", "commit")?>" method="post" style="margin-bottom:50px;">
                  <div class="form-group">
                    <textarea class="form-control" placeholder="提交说明……" name="message" style="width:700px;margin-right:50px;"/></textarea>
                  </div>
                  <input type="submit" class="btn btn-success btn-lg" value="提交修改">
                </form>
        </div>
        <?php else: ?>
        <h3 class="title">本地数据库未做调整</h3>
        <?php endif; ?>
    </div>
</div>
